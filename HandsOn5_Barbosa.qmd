...
```{sql connection=link}
SELECT PNAME, ACRES, ACRES*12/10 as NEW_AREA FROM PRESERVE
```
...
```{sql connection=link}
SELECT PNAME, ACRES, ACRES*1/3 as NEW_AREA FROM PRESERVE
```
...
```{sql connection=link}
SELECT PNAME, FEE, (FEE+50)/2.0 as CHANGED_FEE FROM PRESERVE
```
...
```{sql connection=link}
SELECT substr(PNAME, 1, instr(PNAME || ' ', ' ') - 1) as FIRST_PNAME FROM PRESERVE
ORDER BY PNAME
```
...
```{sql connection=link}
SELECT COUNT(DISTINCT FEE) as DISTINCT_VALUES FROM PRESERVE
WHERE FEE != 0
```
...
```{sql connection=link}
SELECT COUNT(PNAME) as TOTAL_PNAMES, COUNT(DISTINCT PNAME) as DISTINCT_PNAMES FROM PRESERVE
```
...
```{sql connection=link}
SELECT AVG(FEE*(51.0/50.0)) as AVG_FEE_UPDATED FROM PRESERVE
WHERE STATE = "AZ"
```
...
```{sql connection=link}
SELECT ENO, ENAME, SALARY as OLDSALARY, SALARY*11/10 as NEWSALARY FROM EMPLOYEE
```
...
```{sql connection=link}
SELECT SUM(SALARY*11/10) as NS_SUM, 
  AVG(SALARY*11/10) as NS_AVG, 
  MAX(SALARY*11/10) as NS_MAX,
  MIN(SALARY*11/10) as NS_MIN
FROM EMPLOYEE
```
...
```{sql connection=link}
SELECT COUNT(DNO) as D20_EMPLOYEES FROM EMPLOYEE
WHERE DNO = 20
```
...
WHERE (ENAME AND ENO) IS NOT NULL
```
